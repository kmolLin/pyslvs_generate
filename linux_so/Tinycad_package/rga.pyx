...
cdef class Genetic(object):
    ...
    cdef void crossOver(self)except *:
        ...
        for i in range(0, self.nPop-1, 2):
            ...
            if(randV() < self.pCross):
                ...
                # evaluate new baby
                # for baby in self.babyChrom:
                #     baby.f = self.func(baby.v)

                for j in range(3):
                    self.babyChrom[j].f = self.func(self.babyChrom[j].v)
                # for(j = 0;j < 3; j++)
                #     babyChrom[j].f = objFun(babyChrom[j].v);
    ...
    cdef void report(self)except *:
        ...
        #    print("Final Genetik results at %d generations"%(self.gen,))
        #else:
        #    print("Genetik results after %d generations"%(self.gen,))

        #print("Function : %.6f" % (self.chromElite.f,))
        #for i, p in enumerate(self.chromElite.v, start=1):
        #    print('Var %d : %.4f' % (i, p))
